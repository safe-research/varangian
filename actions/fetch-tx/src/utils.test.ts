...
import type { SafeSignature, SafeTransaction } from "shared-utils";
import { assert, describe, expect, it } from "vitest"; // or '@jest/globals'
...
describe("findToExecute", () => {
	it("should throw if multiple transaction are eligible", () => {
		...
		assert.throws(() => {
			findToExecute(txs, VERSION, CHAIN_ID);
		});
	});
	...
	it("should return available transaction", () => {
		...
		const txs: SafeTransaction[] = [expectedTx, buildSafeTx()];
	...
	it("should return specific transaction if tx is available", () => {
		...
		const txs: SafeTransaction[] = [expectedTx, buildSafeTx({ confirmations: [buildSafeSignature()] })];
