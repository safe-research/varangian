...
import { type ExtendedSafeTransaction, getSafeTxHash } from "shared-utils";
...
const checkTransaction = async (
	coSignerMaterial: string,
	safeTx: ExtendedSafeTransaction,
	coSignerSig1271Compatible: boolean,
): Promise<{ coSignerSignature: string; coSignerAddress: string }> => {
...
async function run() {
	try {
		...
		core.setFailed(error instanceof Error ? error.message : "Unknown error occured");
	}
}
